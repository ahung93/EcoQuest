...
public class  UserLoader {
    ...
    public static User createRandomFriend() {
        int randomFirst = (int) ((Math.random() * firstNames.length) % firstNames.length - 1);
        int randomLast = (int) ((Math.random() * lastNames.length) % lastNames.length - 1);
    ...
    public static User createDefaultUser() {
        ...
        completedChallenges.add(new Challenge(Challenge.State.IN_PROGRESS, "Drive 2 Cars", "Drove 2 cars", Challenge.Type.TRANSPORTATION, 6 * 1000 * 60 * 60, 5, 5));
        ...
        user.addNewChallenge(new Challenge(Challenge.State.COMPLETED, "Recycle 2 cans", "Recycled 2 Cans", Challenge.Type.WASTE_AND_ENERGY_REDUCTION, 1 * 1000 * 60 * 60, 3, 5));
        User friend1 = createRandomFriend();
        User friend2 = createRandomFriend();
        User friend3 = createRandomFriend();
